...
L'annonce du poste Développeur·se Front-End au CREALP cite explicitement **Atomic Design** dans les prérequis techniques. Jusqu'au tag `v1.0.0-crealp` (2026-04-22), l'application incarnait la méthodologie sans la documenter visuellement : les préfixes ABEM (`a-`, `m-`, `o-`) et la convention de nommage étaient consignés dans [ADR-002](https://github.com/sodigitaljeremy/alpimonitor/blob/main/docs/architecture/adr/002-abem-methodology.md), mais aucun catalogue consultable n'exposait le design system au recruteur.
...
- **4 molecules, 16 stories** :
  ...
  - `MStationCard` — 4 stories (Federal·light / Research·light / Research·dark·CONFIRMED Bramois / Research·dark·ILLUSTRATIVE Les Haudères), calquées 1:1 sur la production pour démontrer le guard `showSourcingBadge` ([ADR-008](https://github.com/sodigitaljeremy/alpimonitor/blob/main/docs/architecture/adr/008-station-sourcing-transparency.md))
...
- **Tailwind defaults (`emerald-300` / `emerald-400`) pour `ASourcingBadge` variant CONFIRMED**. Respect du parti-pris [ADR-002](https://github.com/sodigitaljeremy/alpimonitor/blob/main/docs/architecture/adr/002-abem-methodology.md) de parcimonie des tokens : introduire un sixième token custom pour un usage unique serait disproportionné. Explicité dans la page `Colors.mdx`.
- **Research stations dans `fr.json`, pas dans le store Pinia**. Dette post-candidature déjà assumée par [ADR-008](https://github.com/sodigitaljeremy/alpimonitor/blob/main/docs/architecture/adr/008-station-sourcing-transparency.md). Les stories d'`OResearchZonesSection` reflètent fidèlement ce trade-off (elles consomment `useI18nList`, comme le composant en prod).
...
- [ADR-002](https://github.com/sodigitaljeremy/alpimonitor/blob/main/docs/architecture/adr/002-abem-methodology.md) — parti-pris ABEM et parcimonie des tokens Tailwind
- [ADR-005](https://github.com/sodigitaljeremy/alpimonitor/blob/main/docs/architecture/adr/005-leaflet-for-mapping.md) — Leaflet pour la cartographie (cadre l'exclusion d'`OStationMap`)
- [ADR-008](https://github.com/sodigitaljeremy/alpimonitor/blob/main/docs/architecture/adr/008-station-sourcing-transparency.md) — sourcing transparency, origine de `ASourcingBadge` et vocabulaire CONFIRMED / ILLUSTRATIVE repris dans les stories `MStationCard`
