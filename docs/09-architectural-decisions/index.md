...
| [ADR-012](adr-012.md) | Introduction d'une couche IA | Acceptée — A+D livrées ; B/C proposées | 2026-06-04 | Narration LLM (Mistral) + observabilité LLM maison (LiteLLM-ready, non déployé) sur `feat/ai-layer` ; anomalie → chat/RAG à venir |
...
- **ADR-012 cadre la couche IA : extensions A (narration Mistral) + D (observabilité LLM maison) livrées** sur `feat/ai-layer` ; B (anomalie) et C (chat/RAG) restent proposées. D livre une observabilité réelle (décorateur → `LlmCallRun`, `/ai/status`) ; le proxy LiteLLM est *ready* mais **non déployé** (point d'extension assumé). C'est avec C que l'« hexagonal » deviendra un objectif réel et localisé (frontière port/adapter). Le statut reflète explicitement le livré vs le à venir.
