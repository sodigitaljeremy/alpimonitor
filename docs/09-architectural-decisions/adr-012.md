...
**Date** : 2026-06-04
**Statut** : Acceptée — **extensions A (narration) + D (observabilité LLM) livrées** sur `feat/ai-layer` ; extensions **B, C proposées** (non implémentées).
...
**B, C** restent au statut **proposé** : leurs hashes seront ajoutés quand elles seront livrées. L'« hexagonal » réel (frontière `QueryPort`/`PrismaQueryAdapter`) arrive avec **C** uniquement.
...
| 3 | **B — Détection d'anomalie** | hook après upsert dans le tick d'ingestion (ou job séparé) ; **réactive le modèle `Alert` déjà présent** (`AlertType.STATISTICAL_ANOMALY` existe déjà) ; `GET /api/v1/alerts` ; `OAlertsPanel` | **Cœur de livraison** |
