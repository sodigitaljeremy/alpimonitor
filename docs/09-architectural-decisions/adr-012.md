...
**Statut** : Acceptée — **extensions A (narration) + D (observabilité LLM) + B (détection d'anomalie) livrées** (A+D sur `feat/ai-layer` ; B sur `feat/ai-anomaly`) ; extension **C (chat sur données structurées) — backend C1-C5 LIVRÉ sur `feat/ai-chat`** (cf. §« Implémentation extension C »), **PR `feat/ai-chat → main` ouverte pour déploiement incrémental** du backend ; restent le front C6 (`OChatPanel` + `useDataChat`) et la doc finale C7.
...
**Implémentation extension C** (branche `feat/ai-chat`, basée sur `main` post-A+D+B) — **backend C1-C5 LIVRÉ** (décisions D1–D7 ci-dessous respectées) :
...
**322 tests verts (203 api + 119 web)**, gate complet OK. **Pas de migration, pas de nouveau secret** (réutilise `MISTRAL_API_KEY` et l'observabilité D). **Restent le front C6** (`OChatPanel` + `useDataChat`, section dédiée scrollable, D7) **et la doc finale C7**. L'« hexagonal » réel (frontière `QueryPort`/`PrismaQueryAdapter`) est désormais matérialisé en C1.
