# ADR-012 — Introduction d'une couche IA (narration, observabilité, anomalie, chat/RAG)

**Date** : 2026-06-04
**Statut** : Acceptée — **extensions A (narration) + D (observabilité LLM) + B (détection d'anomalie) livrées** (A+D sur `feat/ai-layer` ; B sur `feat/ai-anomaly`) ; extension **C (chat/RAG) proposée** (non implémentée).
...
**C** reste au statut **proposé** : ses hashes seront ajoutés quand elle sera livrée. L'« hexagonal » réel (frontière `QueryPort`/`PrismaQueryAdapter`) arrive avec **C** uniquement.
...
| 4 | **C — Chat / RAG sur les données** | module `ai/rag/`, endpoint `POST /api/v1/ask`, **frontière `QueryPort` + `PrismaQueryAdapter`**, function-calling sur requêtes whitelistées, `OChatPanel` + `useDataChat` | **Vitrine stretch** |
...
Plutôt que de saupoudrer le terme partout, la frontière hexagonale (ports/adapters) est introduite **là où elle a une vraie valeur technique** : le chat/RAG (extension C). Un `QueryPort` (interface du domaine) implémenté par un `PrismaQueryAdapter` isole la logique de requêtage de l'infrastructure Prisma. C'est le seul endroit où l'inversion de dépendance se justifie (substituabilité du backend de données, testabilité du raisonnement sans DB). Le reste du backend **reste volontairement plat** (YAGNI, ADR-003).

### Garde-fous transverses

- **Function-calling sur requêtes whitelistées** ≫ SQL/text-to-SQL libre généré par le LLM (sécurité + déterminisme + testabilité).
- **Retrieval = Prisma, pas de vector store** au démarrage : les données sont structurées et de faible volume ; un vector store serait de la sur-ingénierie.
...
- **Retrieval Prisma plutôt que RAG vectoriel** — pivot possible vers un vector store post-MVP si le volume/variété des données l'exige.
