...
Quatre incidents rencontrés en production, quatre post-mortems écrits à chaud. Chaque document suit la structure **TL;DR / Timeline / Symptômes / Diagnostic / Résolution / Prévention / Lessons / Follow-ups** — format cohérent, référence pour les incidents futurs.
...
- **[2026-06-04 — Deploy échoué sur course au recreate Coolify](2026-06-04-deploy-recreate-race.md)** — au merge de la couche IA (A+D), le `docker compose up -d` échoue (exit 255) après suppression des anciens conteneurs → prod 503 ~6 min. Cause : **course transitoire au recreate** (pas le build/code/compose). Résolution : **Redeploy identique** réussi du premier coup. Dette notée : recreate non-atomique = bref downtime possible à chaque deploy.
