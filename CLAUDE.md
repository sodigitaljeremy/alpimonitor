...
**Date dernière mise à jour** : 2026-06-04 (session extension C — **cadrage** chat/RAG sur `feat/ai-chat` ; A+D+B livrés **et en prod**)
...
> Tu reprends pour **cadrer puis livrer l'extension C (chat/RAG)** sur `feat/ai-chat` (branchée sur `main` à jour, B mergé en `aa1d3d8`). **A+D+B sont LIVRÉS ET EN PROD.** Avant de coder : lis `docs/audit/intellitek-audit.md` (carto/gap/mapping/extensions) et [ADR-012](docs/09-architectural-decisions/adr-012.md) (cadrage couche IA, ordre **A→D→B→C**, statuts A/D/B = LIVRÉ, C = la cible décrite §« hexagonal localisé »). Couche IA = additive, isolée ; jamais sur `main` directement, tags `v1.x` protégés.
...
**Prochaine étape — extension C (chat/RAG)** sur `feat/ai-chat`, la **vitrine stretch** (et seul endroit où l'« hexagonal » devient réel) : module `ai/rag/`, `POST /api/v1/ask`, **frontière `QueryPort` + `PrismaQueryAdapter`** (= « hexagonal » localisé, ADR-012), **function-calling sur requêtes Prisma whitelistées** (pas de text-to-SQL libre, pas de vector store — retrieval structuré via tool-use), grounding strict (hors périmètre → « je ne peux pas répondre »), **rate-limit par IP + garde-fou coût quotidien** réutilisant l'observabilité D (`costUsdToday`), `OChatPanel` + `useDataChat`. MVP Q/R simple (pas de multi-tours d'emblée). Point d'arrêt défendable A+D+B **déjà atteint et en prod** : C livrée seulement si le temps le permet. Cadrage C détaillé dans la session courante (à formaliser : mise à jour ADR-012 §C + découpage stories C1..Cn).
