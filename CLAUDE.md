...
**Date dernière mise à jour** : 2026-06-04 (session extension B — détection d'anomalie statistique **livrée sur `feat/ai-anomaly`**, B4 front + doc ; non mergé)
...
> Tu reprends après la **session extension B (détection d'anomalie)** sur `feat/ai-anomaly` : backend B1/B2/B3/B1-bis + **front B4 + doc livrés**, branche **non mergée**. Avant de bosser : lis `docs/audit/intellitek-audit.md` (carto/gap/mapping/extensions) et [ADR-012](docs/09-architectural-decisions/adr-012.md) (cadrage couche IA, ordre **A→D→B→C**, statut B = LIVRÉ). Couche IA = additive, isolée ; jamais sur `main` directement, tags `v1.x` protégés.
...
**B — LIVRÉ sur `feat/ai-anomaly` (PAS encore mergé/déployé).** Détection d'anomalie **statistique, PAS LLM** (module `apps/api/src/anomaly/` séparé). Commits atomiques : `b3ee662` B1 (z-score pur, hystérésis), `d5ca657` B2 (`Alert` réactivé sans migration + scan isolé post-ingestion), `728bf45` B3 (`GET /api/v1/alerts` + `AlertDTO` partagé), `d24af46` B1-bis (**déseasonnalisation horaire** par bucket heure-de-jour UTC, scan 14 j ; CV diurne 0,61→0,30, épisodes 14→15), `353ce24` B4 (front : `useAlertsStore` + façade `useAlertsPanel`, `OAlertsPanel` vue réseau épisodes ouverts + historique + état « réseau nominal », halo carte pulsant + tooltip honnête). Doc B (ce commit) : ADR-012 statut B→LIVRÉ, `backend.md` §5.B.7 + `/alerts`, `persistence.md` `Alert` réactivé + metadata `hourBucket`/`bucketSampleSize`, index ADR. Limites assumées : seuils non calibrés hydrologie experte, étiquetés UI. Gate complet vert (119 web + 157 api). **Seed Valais déjà enrichi à 10 stations LIVE** (`782812a`, avant B1).

**Prochaine étape — extension C (chat/RAG)**, la **vitrine stretch** (et seul endroit où l'« hexagonal » devient réel) :
1. **D'abord** : ouvrir la PR de `feat/ai-anomaly` (B backend + B4 + doc voyagent ensemble), CI verte, merge sur `main`, tag (p.ex. `v1.3.0-anomaly`). Vérifier le scan en prod (épisodes réels sur les 10 LIVE).
2. **C (chat/RAG)** — module `ai/rag/`, `POST /api/v1/ask`, **frontière `QueryPort` + `PrismaQueryAdapter`** (= « hexagonal » localisé, ADR-012), function-calling sur requêtes whitelistées (pas de text-to-SQL libre), retrieval Prisma (pas de vector store), `OChatPanel` + `useDataChat`. Point d'arrêt défendable A+D+B déjà atteint : C livrée seulement si le temps le permet.
