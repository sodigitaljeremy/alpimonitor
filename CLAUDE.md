...
**Date dernière mise à jour** : 2026-06-05 (session C7 — **couche IA COMPLÈTE en prod : A+D+B+C livrés et déployés** ; front C6 mergé (PR #4 `v1.5.0-chat`, PR #5 `v1.5.1-chat-ux`) ; **doc finale C7 en cours sur `feat/ai-docs`** — alignement doc ↔ code, gap audit clos)
...
> Tu reprends sur `feat/ai-docs` (branchée sur `main` à jour) : **la couche IA est COMPLÈTE et en prod — A + D + B + C livrés et déployés** (C backend PR #3 `v1.4.0-chat-api`, C front C6 PR #4 `v1.5.0-chat`, polish UX PR #5 `v1.5.1-chat-ux`). Il ne reste **que la doc C7** (cette session) : **C7a** = doc « haute » (ADR-012 statut C complet + hashes C6 ; README archi hexagonale localisée + features couche IA ; CLAUDE pickup) — **FAIT** ; **C7b** = arc42 détaillé (building blocks §5 chat + `QueryPort`/`PrismaQueryAdapter` ; runtime §6 flux chat ; cross-cutting §8 grounding/observabilité/rate-guard). STATUS/PRD live n'existent plus (archivés sous `docs/_legacy/`, gelés) → rien à reconcilier là, l'état vit dans ADR-012 + arc42 + ce pickup. Avant de coder : lis [ADR-012](docs/09-architectural-decisions/adr-012.md) (à jour) et `docs/audit/intellitek-audit.md`. Couche IA = additive, isolée ; jamais sur `main` directement, tags `v1.x` protégés. **C7 = QUE de la doc, aucun code applicatif.**

**A+D — LIVRÉ EN PROD.** Mergé via PR #1 en merge commit **`d0fa304`** sur `main`, tag **`v1.2.0-ai`**. A : `Insight` (cache idempotent), `computeNarrationFeatures` (pur, groundé), client **Mistral** derrière `LlmClient`, `GET /stations/:id/narrative`, façade + bouton « Générer le résumé ». D : décorateur `ObservingLlmClient` → `LlmCallRun` (coût/latence/tokens), `computeCostUsd`, `GET /ai/status`, badge IA global, compteurs d'ingestion honnêtes (`created`/`unchanged`, D0). **LiteLLM non déployé** (client LiteLLM-ready via `LITELLM_BASE_URL`). Secret prod `MISTRAL_API_KEY` côté Coolify.

**B — LIVRÉ EN PROD.** Mergé via PR #2 en merge commit **`aa1d3d8`** sur `main`, tag **`v1.3.0-anomaly`**. Détection d'anomalie **statistique, PAS LLM** (module `apps/api/src/anomaly/` séparé). Commits atomiques : `b3ee662` B1 (z-score pur, hystérésis), `d5ca657` B2 (`Alert` réactivé sans migration + scan isolé post-ingestion), `728bf45` B3 (`GET /api/v1/alerts` + `AlertDTO` partagé), `d24af46` B1-bis (**déseasonnalisation horaire** par bucket heure-de-jour UTC, scan 14 j ; CV diurne 0,61→0,30, épisodes 14→15), `353ce24` B4 (front : `useAlertsStore` + façade `useAlertsPanel`, `OAlertsPanel` vue réseau + halo carte + tooltip honnête), `5335378` doc B. Limites assumées : seuils non calibrés hydrologie experte, étiquetés UI. **Seed Valais à 10 stations LIVE** (`782812a`).

**Extension C — COMPLÈTE EN PROD (C1-C7)**. Backend C1-C5 mergé via PR #3 (merge commit **`b743bd6`**) sur `main`, tag **`v1.4.0-chat-api`** ; front C6 mergé via PR #4 (tag **`v1.5.0-chat`**) + polish UX via PR #5 (tag **`v1.5.1-chat-ux`**) ; doc C7 sur `feat/ai-docs`. **Validé end-to-end** sur l'endpoint public. La **vitrine stretch — livrée** (et seul endroit où l'« hexagonal » devient réel). Commits atomiques : `f1c7b44` C1 (frontière **`QueryPort` + `PrismaQueryAdapter`**, backend pur, zéro LLM), `9a1f03f` C2 (tool-calling sur `LlmClient` + observabilité chat, operation `'chat'` réutilisant D), `57a4a93` C3a (specs des 4 tools + dispatcher + prompt grounding-strict) + `ebffb6a` C3b (orchestrateur chat — boucle tool bornée 2 rounds), `41641e6` C4 (**rate-guard unifié** maison : cost-cap $0,50/j + fenêtres fixes par IP 5/min & 20/j), `b9c22ce` C5 (endpoint public **`POST /api/v1/ask`** — 429/400/200/503 gracieux). **function-calling Mistral sur 4 fonctions whitelistées** (`find_stations`, `get_latest_measurements`, `get_measurement_stats`, `list_alerts` ; PAS de text-to-SQL, PAS de vectoriel), grounding strict (hors périmètre → « je ne peux pas répondre »). Déployé **sans migration et sans nouveau secret**. **Front C6 livré** : façade `useDataChat` + `api-client.ask` (`b84655b` C6a), organisme `OChatPanel` section dédiée scrollable avec trace des tools + disclaimer (`92a4367` C6b), puis polish UX (`7caeef3` chat-v2, `fe4cea0` découvrabilité, `2f4df8f` chat-v3). **344 tests verts (208 api + 136 web).** Point d'arrêt défendable A+D+B atteint et en prod ; C l'est désormais aussi de bout en bout. Reste **C7** (doc finale, cette session).

**Env dev couche IA** : postgres dev (séries réelles 24 h importées de la prod pour les LIVE) — `docker compose down` possible, l'ingestion réelle reremplit au besoin.

**Tags** : `v1.0.0-crealp`, `v1.1.0-refactor`, `v1.2.0-ai` (← A+D), `v1.3.0-anomaly` (← B). Le point d'arrêt défendable A+D+B est atteint **et en prod** ; C est un bonus stretch.
