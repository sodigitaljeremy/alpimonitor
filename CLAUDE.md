...
**Date dernière mise à jour** : 2026-06-05 (session extension C — **backend C1-C5 LIVRÉ** sur `feat/ai-chat`, PR en cours pour déploiement incrémental ; A+D+B en prod)
...
> Tu reprends sur `feat/ai-chat` : **le backend de l'extension C (chat sur données structurées) est LIVRÉ — stories C1 à C5 complètes, 322 tests verts (203 api + 119 web), gate complet OK**. Une **PR `feat/ai-chat → main` est ouverte** pour le **déploiement incrémental** du backend C. Restent **C6 (front `OChatPanel` + `useDataChat`, section dédiée scrollable)** et **C7 (doc finale + reconcile STATUS/PRD)**. Avant de coder : lis `docs/audit/intellitek-audit.md` (carto/gap/mapping/extensions) et [ADR-012](docs/09-architectural-decisions/adr-012.md) §« Cadrage détaillé extension C » (décisions D1–D7 figées, statuts A/D/B = LIVRÉ, backend C1-C5 = LIVRÉ). Couche IA = additive, isolée ; jamais sur `main` directement, tags `v1.x` protégés.
...
**Extension C — BACKEND LIVRÉ (C1-C5)** sur `feat/ai-chat`, la **vitrine stretch** (et seul endroit où l'« hexagonal » devient réel). Commits atomiques : `f1c7b44` C1 (frontière **`QueryPort` + `PrismaQueryAdapter`**, backend pur, zéro LLM), `9a1f03f` C2 (tool-calling sur `LlmClient` + observabilité chat, operation `'chat'` réutilisant D), `57a4a93` C3a (specs des 4 tools + dispatcher + prompt grounding-strict) + `ebffb6a` C3b (orchestrateur chat — boucle tool bornée 2 rounds), `41641e6` C4 (**rate-guard unifié** maison : cost-cap $0,50/j + fenêtres fixes par IP 5/min & 20/j), `b9c22ce` C5 (endpoint public **`POST /api/v1/ask`** — 429/400/200/503 gracieux). **function-calling Mistral sur 4 fonctions whitelistées** (`find_stations`, `get_latest_measurements`, `get_measurement_stats`, `list_alerts` ; PAS de text-to-SQL, PAS de vectoriel), grounding strict (hors périmètre → « je ne peux pas répondre »). **PR `feat/ai-chat → main` ouverte pour déploiement incrémental** du backend (PAS de migration, PAS de nouveau secret). **Reste : C6** (front `OChatPanel` + `useDataChat`, section dédiée scrollable) **+ C7** (doc finale). Point d'arrêt défendable A+D+B **déjà atteint et en prod** ; le backend C est un bonus déployable seul, le front C6 suit.
